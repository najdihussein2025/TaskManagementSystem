...
namespace TaskManagementSystem.Interfaces.Services
{
    ...
    {
    Task<(bool Success, string? Error)> CreateAsync(CreateUserDto dto);//create user using dto
        ...
        Task<(bool Success, string? Error)> UpdateAsync(UpdateUserDto dto);//update user using dto
